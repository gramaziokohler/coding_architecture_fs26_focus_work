...
def _ray_obb_exit(origin, direction, beam):
    """Distance from `origin` along `direction` to where the ray leaves the
    beam's oriented bounding box (its solid). Returns None if it misses.
    Used to land a screw head on the beam's real surface, not an infinite plane."""
    cl = beam.centerline
    cen = cl.midpoint
    ...
    if tmax < tmin:
        return None
    return tmax  # far exit, i.e. the surface on the +direction side

class DrillingProcessor:
    def __init__(self, timber_model, screw_diameter=0.006, screw_length=0.150, screw_spacing=0.040, max_drilling_depth=None, run_joinery=True, max_arch_penetration=None):
        ...
        # Caps how far a foundation screw head runs UP into the arch above the
        # contact. None -> cap at the arch's own height (one beam-depth), which
        # keeps shallow joints flush at the surface and stops steep ones from
        # producing very long screws. Set a value in metres to override.
        self.max_arch_penetration = max_arch_penetration   # NEW
    ...
    def _apply_foundation_butt_drilling(self, joint, abut_beam, cont_beam):
        ...
        # How far the head may run UP into the arch above the contact.
        # Cap at the arch height (one beam-depth) unless overridden. This keeps
        # shallow joints flush at the surface and stops steep arches from
        # producing very long screws.
        ...
        for sign in (offset_vec, -offset_vec):
            ...
            target_tail = contact_pt + screw_dir * 0.080      # 80 mm into foundation (THE BOTTOM POINT)

            # Head = where the screw exits the arch's real surface (ray vs the
            # actual beam solid), but never more than `penetration_cap` above
            # the contact.
            exit_dist = _ray_obb_exit(contact_pt, face_n, abut_beam)
            if exit_dist is None or exit_dist <= 1e-6:
                # Fallback: intersect the top-face plane
