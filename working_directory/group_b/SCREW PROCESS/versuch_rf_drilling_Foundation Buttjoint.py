...
class DrillingProcessor:
    ...
    def _apply_standard_butt_drilling(self, joint, beam1, beam2, cat1, cat2):
        ...
        # Extract exact intersection points on both centerlines
        pt_a = Point(res[0][0], res[0][1], res[0][2])
        pt_c = Point(res[1][0], res[1][1], res[1][2])
        ...
        # dir_s points strictly INTO Beam A along its centerline
        ...
        # Direction of Beam B (continuous beam)
        dir_c = line_c.direction.copy()
        dir_c.unitize()

        # Calculate exact hypotenuse distance to traverse Beam B diagonally
        thickness_c = max(cont_beam.width, cont_beam.height)
        cos_alpha = abs(dir_s.dot(dir_c))
        sin_alpha = math.sqrt(max(0.0, 1.0 - cos_alpha**2))
        
        perpendicular_clearance = (thickness_c / 2.0) + dist_centers
        
        # Traverse distance backwards along the slanted dir_s vector
        traversal_b = perpendicular_clearance / max(0.1, sin_alpha)
        
        # Enter at least 8 cm deeply into Beam A
        anchor_depth = 0.080
        req_screw_length = traversal_b + anchor_depth
        
        # Ceil to the nearest 10mm to guarantee at least 8cm depth is met physically
        req_screw_length = math.ceil(req_screw_length / 0.010) * 0.010
        actual_anchor_depth = req_screw_length - traversal_b
        ...
        # Drill starts flush with the outside of Beam B and ends inside Beam A
        cnc_start = pt_a - (dir_s * (traversal_b + tolerance))
        cnc_end = pt_a + (dir_s * (actual_anchor_depth + tolerance))
        
        cnc_line_1 = Line(cnc_start + offset_vec, cnc_end + offset_vec)
        cnc_line_2 = Line(cnc_start - offset_vec, cnc_end - offset_vec)

        hw_start = pt_a - (dir_s * traversal_b)
        hw_end = pt_a + (dir_s * actual_anchor_depth)
    ...
    def _apply_standard_butt_drilling(self, joint, beam1, beam2, cat1, cat2):
        ...
        # We need to traverse from the center of Beam B out to its faces.
        thickness_c = max(cont_beam.width, cont_beam.height)
        
        # The distance from centerline to ONE face along the diagonal drill vector:
        traversal_half = (thickness_c / 2.0 + dist_centers) / max(0.1, sin_alpha)
        
        # Enter exactly 8 cm deeply into Beam A AFTER exiting Beam B
        anchor_depth = 0.080
        
        # Total screw length = Traversing ENTIRE Beam B (2 * half) + anchor_depth
        req_screw_length = (2.0 * traversal_half) + anchor_depth
        
        # Ceil to the nearest 10mm to guarantee hardware procurement length
        req_screw_length = math.ceil(req_screw_length / 0.010) * 0.010
        
        # Since we rounded up, the extra length pushes deeper into Beam A
        ...
        # Drill starts flush with the FAR face of Beam B 
        # pt_a is roughly the center of the joint.
        # Moving BACKWARDS by traversal_half puts us on the far side of Beam B.
        cnc_start = pt_a - (dir_s * (traversal_half + tolerance))
        
        # Moving FORWARDS by (traversal_half + actual_anchor_depth) puts us inside Beam A.
