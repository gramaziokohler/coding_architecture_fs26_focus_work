...
def run_numbering(timber_model, Index, RunExport, OutputFolder):
    ...
    # =========================================================================
    # POST-PROCESSING: SPOSTAMENTO ELEMENTI (dal secondo codice)
    # =========================================================================
    ...
    # Seconda passata: inserimento nei moduli target
    for initial_name, target_mod in manual_moves.items():
        if initial_name in guid_by_initial_name:
            node_to_move = guid_by_initial_name[initial_name]
    ...
    for k in labels_keys:
        ...
        for i, node in enumerate(ordered):
            ...
            # Verifica key beam basandosi sul nome originario
    ...
    for k in labels_keys:
        for node in ordered_by_module.get(k, []):
            ...
            # Tag [KEY BEAM] nelle info se applicabile
    ...
    # =========================
    # 10. EXPORT DINAMICO
    # (struttura cartelle dal PRIMO codice: output_folder/beams/beam_id/)
    # =========================
    if RunExport and OutputFolder:
        ...
        for k in labels_keys:
            for node in ordered_by_module.get(k, []):
                ...
                # *** STRUTTURA CARTELLE DAL PRIMO CODICE ***
                ...
                # *** JSON CON is_key_beam DAL SECONDO CODICE ***
                beam_data = {
                    "beam ID": beam_id,
                    "name": name,
                    "module": k,
        ...
        # Global structure con is_key_beam
        all_beams_list = []
        for k in labels_keys:
            for node in ordered_by_module.get(k, []):
