...
def solve_bin_packing(timber_model, stock_length, saw_kerf=0.0, beam_side_gap=0.01):
    """
    Bin packing con spazio orizzontale a sinistra e a destra di ogni beam.

    beam_side_gap:
        spazio libero prima e dopo ogni beam.
        Se il modello è in metri:
        0.01 = 1 cm.

    Ogni beam occupa quindi:
        beam_length + 2 * beam_side_gap + saw_kerf
    """
    ...
    for i, beam in enumerate(beams):
        ...
        needed_len = length + 2.0 * beam_side_gap + saw_kerf
        ...
        for stock in stocks:
            ...
            if stock["remaining"] >= needed:
                ...
                stock["beams"].append({
                    ...
                    "visual_start_pos": current_start + beam_side_gap,
                    "length": item["length"],
                    "needed_len": needed
                })
                ...
                "beams": [
                    {
                        "beam": item["beam"],
                        "start_pos": 0.0,
                        "visual_start_pos": beam_side_gap,
                        "length": item["length"],
                        "needed_len": needed
                    }
                ],
...
def visualize_packing(stocks, origin, stock_length, beam_spacing=0.6, beam_offset=0.2):
    """
    Visualizza gli stock e i beams.

    La posizione X del beam usa visual_start_pos, cioè:
        start_pos + beam_side_gap

    Così il beam reale parte dopo il margine laterale.
    """
    ...
    for i, stock in enumerate(stocks):
        ...
        for item in stock["beams"]:
            ...
            if "visual_start_pos" in item:
                start_pos = item["visual_start_pos"]
            else:
                start_pos = item["start_pos"]

            start_x = base_x + start_pos
            ...
            X = Transformation.from_frame_to_frame(
                source_frame,
                target_frame
            )
...
def solve_grouped_bin_packing(timber_model, stock_length, saw_kerf=0.0, beam_side_gap=0.01):
    """
    Runs bin packing separately for every cross-section / Querschnitt.

    beam_side_gap aggiunge spazio a sinistra e destra di ogni beam.
    """
...
def solve_and_visualize_grouped_packing(
    ...
    beam_side_gap:
        spazio libero a sinistra e destra di ogni beam.
        0.01 = 1 cm, se il modello è in metri.
    """
