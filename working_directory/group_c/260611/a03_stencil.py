...
def create_stencil(
    ...
    """Create plates, frame beams, plate beams, lap joints, and preview geometry."""

    rectangles = rectangles or []
    points = points or []

    # Backwards compatibility:
    # se usi ancora il vecchio input "lines", questi diventano frame beams.
    if frame_beam_lines is None:
        frame_beam_lines = lines or []
    else:
    ...
    plates_out = [
        element_geometry(plate, geometry_errors, compute_plate_geometry)
        for plate in plates
    ]
    ...
    beams_out = frame_beams_out + plate_beams_out
    ...
    frame_beams_rhino = [
        rhino_geometry(geometry, geometry_errors)
        for geometry in frame_beams_out
    ]

    plate_beams_rhino = [
        rhino_geometry(geometry, geometry_errors)
        for geometry in plate_beams_out
    ]

    beams_rhino = frame_beams_rhino + plate_beams_rhino
