...
const getBeamFrame = (beamData = currentBeamData) => beamData?.local_frame || beamData?.frame || null;
...
const getBeamModelUrl = (beamDataOrId) => {
    ...
    return resolveModelUrl(beamData?.geometry_model || beamData?.["3d_model"] || `beams/${id}/${id}.stl`);
};

const getBlankModelUrl = (beamDataOrId) => {
    const beamData = typeof beamDataOrId === "object" ? beamDataOrId : beamDataCache.get(beamDataOrId);
    const id = typeof beamDataOrId === "string" ? beamDataOrId : getBeamId(beamData);
...
const getCenterlineDirection = (beamData = currentBeamData) => {
    const position = getGlobalPosition(beamData);
...
const computeBlankFrameOrigin = (beamData, blankGeometry) => {
    const frame = getBeamFrame(beamData);
    if (!frame?.x_axis || !frame?.y_axis || !frame?.z_axis || !blankGeometry?.attributes?.position) return null;

    const frameOrigin = vectorFromArray(frame.origin || [0, 0, 0]);
    const xAxis = getCenterlineDirection(beamData) || vectorFromArray(frame.x_axis).normalize();
    const yAxis = vectorFromArray(frame.y_axis).normalize();
    const zAxis = vectorFromArray(frame.z_axis).normalize();
    const position = blankGeometry.attributes.position;
    ...
    for (let i = 0; i < position.count; i += 1) {
        ...
        const local = point.clone().sub(frameOrigin);
        const x = local.dot(xAxis);
        const y = local.dot(yAxis);
        const z = local.dot(zAxis);
    ...
    const faceCenter = (x) => frameOrigin
        .clone()
        .add(xAxis.clone().multiplyScalar(x))
        .add(yAxis.clone().multiplyScalar(midY))
        .add(zAxis.clone().multiplyScalar(midZ));

    const start = getGlobalPosition(beamData)?.centerline_start;
    if (!start) return faceCenter(minX);

    const startPoint = vectorFromArray(start);
    const minFace = faceCenter(minX);
    const maxFace = faceCenter(maxX);
    return minFace.distanceToSquared(startPoint) <= maxFace.distanceToSquared(startPoint) ? minFace : maxFace;
};

const loadBlankGeometry = async (beamData) => {
    const url = getBlankModelUrl(beamData);
    if (!url) return null;
    return loadSTL(url);
};
...
const addBlankOverlay = async (beamData) => {
    if (!showBlankBeams.value || !beamData) return;
    try {
        const blankGeometry = await loadBlankGeometry(beamData);
        if (!blankGeometry) return;
        ...
        const mesh = new THREE.Mesh(blankGeometry, material);
        ...
        console.warn(`Could not load blank STL for ${getBeamId(beamData)}`, e);
    }
};
...
const isKeyBeam = (beamData) => {
    return beamData?.is_key_beam === true || beamData?.key_beam === true;
};
...
const loadSingleBeam = async ({ preserveCamera = false } = {}) => {
    ...
    const geometry = await loadSTL(getBeamModelUrl(currentBeamData));
    geometry.computeBoundingBox();
    const size = new THREE.Vector3();
